    ...
    if ($result) {
        ...
        if (isset($_FILES["img"]) && $_FILES["img"]["error"] === 0 && isset($_FILES["banner"]) && $_FILES["banner"]["error"] === 0) {
            ...
            if ($upload_success) {
                ...
                $update_query = "UPDATE `userreg` SET `username`='$name', `usernumber`='$number', `useremail`='$email', `userpass`='$password', `userprofile`='$filepath', `userbanner`='$bannerpath' WHERE userid = $userid";
